import { jsx, jsxs, Fragment } from "react/jsx-runtime";
import { useState, useRef, useEffect } from "react";
import { motion } from "framer-motion";
import { H as HeaderComponent, F as FooterComponent } from "./Header-CrWG846T.js";
import { BiCaretDown, BiSearch } from "react-icons/bi";
import { ToastContainer } from "react-toastify";
import { U as UserLoggedProvider } from "./loggedUser-DyDIPP3j.js";
import { F as FormStateProvider } from "./stateForm-DYypFJ1f.js";
import { useForm } from "@inertiajs/react";
import axios from "axios";
import "react-icons/ai";
import "react-icons/bs";
import "react-icons/fa";
import "./index-DJUNAe3r.js";
import "react-icons/fa6";
import "react-icons/ri";
import "react-icons/fi";
const newCompanyService = (plan, local) => {
  const [showCountryDropdown, setShowCountryDropdown] = useState(false);
  const [showActivityDropdown, setShowActivityDropdown] = useState(false);
  const { data, setData, post, processing, errors } = useForm({
    imagem: "data:image/png;base64,",
    name: "",
    nif: "",
    phone: "",
    email: "",
    country: "",
    city: "",
    activity: "",
    id: plan
  });
  const handleSubmit = (e) => {
    e.preventDefault();
    post(`/register/company`);
  };
  const handleSelectCountry = (country) => {
    setData("country", country.name);
    setShowCountryDropdown(false);
  };
  const handleSelectActivity = (activity) => {
    setData("activity", activity.name);
    setShowActivityDropdown(false);
  };
  return {
    data,
    setData,
    processing,
    setShowActivityDropdown,
    errors,
    handleSubmit,
    handleSelectCountry,
    handleSelectActivity,
    showActivityDropdown,
    showCountryDropdown,
    setShowCountryDropdown
  };
};
function CompanyRegistration({ plan, auth, local }) {
  const {
    data,
    setShowActivityDropdown,
    errors,
    processing,
    setData,
    handleSelectActivity,
    handleSelectCountry,
    handleSubmit,
    showActivityDropdown,
    setShowCountryDropdown,
    showCountryDropdown
  } = newCompanyService(plan);
  const countries = useRef([]);
  const activities = useRef([]);
  useEffect(() => {
    (async () => {
      await axios.get("/data/country.json").then((response) => {
        countries.current = response.data;
      }).catch((err) => {
        console.log(err);
      });
      await axios.get("/data/activities.json").then((response) => {
        activities.current = response.data;
      }).catch((err) => {
        console.log(err);
      });
    })();
  }, []);
  return /* @__PURE__ */ jsx(UserLoggedProvider, { children: /* @__PURE__ */ jsxs(FormStateProvider, { children: [
    /* @__PURE__ */ jsx(HeaderComponent, { auth }),
    /* @__PURE__ */ jsxs(
      motion.div,
      {
        initial: { opacity: 0 },
        animate: { opacity: 1 },
        transition: { duration: 0.5 },
        className: "max-w-4xl mt-28 mb-10 mx-auto bg-white p-8 rounded-lg shadow mt-12",
        children: [
          /* @__PURE__ */ jsx("h1", { className: "text-2xl font-bold mb-4", children: "Cadastro de Empresa" }),
          /* @__PURE__ */ jsx("p", { className: "mb-8", children: "Preencha as informações abaixo para registrar sua empresa no nosso sistema e começar a desfrutar de todos os recursos disponíveis." }),
          /* @__PURE__ */ jsxs("form", { onSubmit: handleSubmit, className: "space-y-6", children: [
            /* @__PURE__ */ jsx(InputField, { type: "text", errors, label: "Nome", name: "name", value: data.name, onChange: (e) => setData("name", e.target.value) }),
            /* @__PURE__ */ jsx(InputField, { type: "text", errors, label: "Identificação Fiscal (NIF)", name: "nif", value: data.nif, onChange: (e) => setData("nif", e.target.value) }),
            /* @__PURE__ */ jsx(InputField, { type: "text", errors, label: "Telefone", name: "phone", value: data.phone, onChange: (e) => setData("phone", e.target.value) }),
            /* @__PURE__ */ jsx(InputField, { label: "E-mail", errors, type: "email", name: "email", value: data.email, onChange: (e) => setData("email", e.target.value) }),
            /* @__PURE__ */ jsx(
              DropdownSelect,
              {
                errors,
                label: "País",
                placeholder: "Escolha seu país",
                value: data.country,
                showDropdown: showCountryDropdown,
                setShowDropdown: setShowCountryDropdown,
                items: countries.current,
                onSelectItem: handleSelectCountry
              }
            ),
            /* @__PURE__ */ jsx(
              DropdownSelect,
              {
                errors,
                label: "Tipo de Atividade",
                placeholder: "Escolha a atividade",
                value: data.activity,
                showDropdown: showActivityDropdown,
                setShowDropdown: setShowActivityDropdown,
                items: activities.current,
                onSelectItem: handleSelectActivity
              }
            ),
            /* @__PURE__ */ jsx(InputField, { errors, type: "text", label: "Cidade", name: "city", value: data.city, onChange: (e) => setData("city", e.target.value) }),
            /* @__PURE__ */ jsx(
              motion.button,
              {
                whileHover: { scale: 1.05 },
                whileTap: { scale: 0.95 },
                className: "btn btn-primary",
                disabled: processing,
                children: "Cadastrar"
              }
            )
          ] }),
          /* @__PURE__ */ jsx(ToastContainer, {})
        ]
      }
    ),
    /* @__PURE__ */ jsx(FooterComponent, {})
  ] }) });
}
function InputField({ label, name, errors, type, value, onChange }) {
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsxs("div", { children: [
      /* @__PURE__ */ jsx("label", { htmlFor: name, className: "block text-sm font-medium text-gray-700", children: label }),
      /* @__PURE__ */ jsx(
        "input",
        {
          type,
          name,
          id: name,
          value,
          onChange,
          className: "mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
        }
      )
    ] }),
    errors[name] != null && /* @__PURE__ */ jsx("span", { className: "text-red-600", children: errors[name] })
  ] });
}
function DropdownSelect({ label, placeholder, value, showDropdown, errors, setShowDropdown, items, onSelectItem }) {
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredItems, setFilteredItems] = useState(items);
  useEffect(() => {
    const results = items.filter(
      (item) => item.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
    setFilteredItems(results);
  }, [searchTerm, items]);
  return /* @__PURE__ */ jsxs("div", { className: "relative", children: [
    /* @__PURE__ */ jsx("label", { className: "block text-sm font-medium text-gray-700", children: label }),
    /* @__PURE__ */ jsxs("div", { className: "mt-1", children: [
      errors[items.name] != null && /* @__PURE__ */ jsx("span", { className: "text-red-600", children: errors[items.name] }),
      /* @__PURE__ */ jsxs("button", { type: "button", className: "input input-bordered w-full text-left border rounded-md p-2 flex justify-between items-center", onClick: () => setShowDropdown(!showDropdown), children: [
        value || placeholder,
        " ",
        /* @__PURE__ */ jsx(BiCaretDown, {})
      ] }),
      showDropdown && /* @__PURE__ */ jsxs("div", { className: "absolute z-10 w-full bg-white mt-1 rounded-md shadow-lg", children: [
        /* @__PURE__ */ jsx("div", { className: "p-2", children: /* @__PURE__ */ jsxs("div", { className: "flex items-center border-b border-gray-300", children: [
          /* @__PURE__ */ jsx(BiSearch, { className: "text-gray-500 mr-2" }),
          /* @__PURE__ */ jsx(
            "input",
            {
              type: "text",
              className: "input input-bordered w-full p-2",
              placeholder: "Pesquisar...",
              onChange: (e) => setSearchTerm(e.target.value)
            }
          )
        ] }) }),
        /* @__PURE__ */ jsxs("div", { className: "max-h-60 overflow-auto", children: [
          filteredItems.map((item) => /* @__PURE__ */ jsx("div", { className: "p-2 hover:bg-gray-100 cursor-pointer", onClick: () => onSelectItem(item), children: item.name }, item.name)),
          filteredItems.length === 0 && /* @__PURE__ */ jsx("div", { className: "p-2 text-gray-500", children: "Nenhum item encontrado." })
        ] })
      ] })
    ] })
  ] });
}
export {
  CompanyRegistration as default
};
